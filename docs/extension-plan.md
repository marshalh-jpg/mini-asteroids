...
- [ ] **Levels.** Add a `level` field to `Game`, starting at 1. On asteroid
...
- [ ] **Saucer enemy.** Add a saucer object type, separate from asteroids.
...
- [ ] **Saucer collisions and scoring.** A player bullet that hits the
...
- [ ] **Power-ups.** On each asteroid destroy event, roll
...
- [ ] **Power-up effects and pickup.** A ship-to-power-up collision
...
- [ ] **Client rendering and HUD.** Update `public/game.js` to draw the
...
- [ ] **Docker verification and running instructions.** Confirm
